...
# Perspectives on Generative Artificial Intelligence {background-iframe="hello-matrix/index.html"} 
...
## :hand: What does my role mean in practice?

I develop technical solutions that minimise the risk of using GenAI models and try to avoid scenarios where Generative AI applications go "rogue", e.g. 
...
## :hand: What will we cover in this presentation?

:one: Demystifying jargon

:two: Brief history of how we got here

:three: Where are we going next

:four: Business implications
...
![](img/robot-graduating.png){fig-align="center" height=300}
...
<p class="footnote">
_image generated using [Gemini Nano Banana 2](https://gemini.google/overview/image-generation/) with a prompt: `a robot during their graduation ceremony looking happy`_<p>

## :pager: Machine Learning: an example

![](img/sentiment-analysis.png){fig-align="center" height=350}
...
<p class="footnote">
_image generated using [Gemini Nano Banana 2](https://gemini.google/overview/image-generation/) with a prompt: `a dashboard with three emojis: angry, neutral, happy`_<p>
...
::::{.columns}
::: {.column width=50%}
![](img/execs_cover.png){fig-align="center" height=600}
:::
...
display(Audio("img/exec_jingle.mp3"))
```

```{yaml}{.scrollable}
[Verse 1]
Thursday morning, June four
Borough Road, we on the floor
The Ministry, doors swing wide
Global minds on one side

Arrival eight-thirty, coffee cups
Big ideas start lining up
HR leaders in the room
Talkin' growth, talkin' moves

[Pre-Chorus]
GenAI in the enterprise
Clear eyes, real use, real size
Talent plans, service game
Everybody there for the same aim

[Chorus]
At The Ministry, we build
At The Ministry, we build
AI, talent, service excellence
All that pressure turn to skill

At The Ministry, we connect
At The Ministry, we connect
Same room, same mission
Different lanes, same respect

[Verse 2]
Dr Mac break it down clean
Machine learning made seen
Red Hat voice, practical
Every answer tactical

Cath talk access to the talent
Mobility with the business in balance
AI agent in the loop
Making hard moves look smooth

[Pre-Chorus]
Kate brings the service truth
Lessons learned for the whole crew
Senior voices, honest talk
You can hear the future walk
...
[Bridge]
Optional lunch, stay around
Keep the conversation grounded
Questions up, cards on the table
Every seat in the room stay able

Global coverage, one strong thread
From the morning into what comes next
Borough Rd on your mind
Real work, real time
...
<p class="footnote">
_audio generated using [Suno AI](https://app.suno.ai/) with a prompt: `take a look at this [event description](https://www.k2group.com/en-us/insights/london-mobility-leadership-morning) and turn it into a Hip-Hop and R&B song`; image generated with [Gemini Nano Banana 2](https://gemini.google/overview/image-generation/) with a prompt: `execs and machine learning engineers on an rap album cover`_<p>
...
## :clock230: Timeline: "ancient" and "recent" history 

While __GenAI__ models have been around for a while, their recent success exploded in 2023

::: {.r-stack}

![](img/gen-ai-timeline.jpeg){.fragment fig-align="center" height=550}

![](img/gen-ai-timeline-recent.png){.fragment fig-align="center" height=550}
:::

<p class="footnote">
_first picture taken from [David Foster](https://www.linkedin.com/feed/update/urn:li:activity:7044233450295316480/); second picture generated with a [Gemini Nano Banana 2](https://gemini.google/overview/image-generation/)_<p>
...
## :bulb: Breakthrough 5: assistants that can reason

![](img/reasoning.jpg){fig-align="center" height=450}

Encouraging models to explain their reasoning process seems to improve performance their ability to perform deduction and make decisions

<p class="footnote">_figure taken from [Understanding Reasoning LLMs
](https://magazine.sebastianraschka.com/p/understanding-reasoning-llms)_<p>

## :bulb: Breakthrough 6: tool use

In a _typical_ conversation with a model through an API, the conversation will alternate between user and assistant messages:
...
But you can extend this conversation to include other message types, for example:
...
# :rocket: Bonus -- quick wins

## :rocket: Choosing the right model
...
## :rocket: Prompt engineering

[Lilian Weng](https://lilianweng.github.io/posts/2023-03-15-prompt-engineering/) defines __prompt engineering__ as: 

> methods for how to communicate with an LLM to steer its behavior for desired outcomes without updating the model weights

A __prompt__ contains any of the [following](https://www.promptingguide.ai/introduction/elements): 

- __input__ -- the text which is fed to the model to generate a sequence of most likely tokens
- __instruction__ -- a specific task for a model to perform
- __context__ -- external information that can steer the model towards a desired outcome
- __output indicator__ -- the type or format of the desired response

## :rocket: Prompt engineering

There are several key strategies for prompt engineering including:

- basic prompting:
    - zero-shot 
    - few-shot
- instruction prompting
- chain- / tree-of-thought prompting

For a comprehensive overview of different prompting strategies, visit e.g. [this guide](https://cloud.google.com/discover/what-is-prompt-engineering)

##  :chart_with_upwards_trend: Model performance can be sensitive to the choice of prompt strategy 

::: {.r-stack}
![](img/zs-vs-zs-cot.png){.fragment .fade-in-then-out width="1000" height="500"}

![](img/cot-table.png){.fragment width="750" height="600"}
:::

<p class="footnote">_figures taken from [Large Language Models are Zero-Shot Reasoners](https://arxiv.org/pdf/2205.11916.pdf)_<p>

## :zap: Recommendations for writing prompts

When developing prompts, the following set of recommendations can be useful:

- try to be specific and precise and avoiding say “not do something” but rather specify what to do
- if the requirement is complex, break it down into multiple prompts
- combine it with other prompt engineering techniques, such as few-shot prompting

OpenAI has published a blog on best practices for [prompt engineering](https://help.openai.com/en/articles/6654000-best-practices-for-prompt-engineering-with-openai-api) for their suite of models, which is worth a read

## :zap: Recommendations for writing prompts contd.

Additionally, it is prudent to:

- use [prompt templates](https://python.langchain.com/docs/modules/model_io/prompts/prompt_templates/) to ensure consistency and reproducibility
- consider using multiple prompts to generate multiple candidate answers and produce final answer by either the majority vote ([self-consistency](https://arxiv.org/abs/2203.11171)) or pick the best single answer
- understand the risks of [prompt injection attacks](https://arxiv.org/abs/2302.12173) and how to mitigate them

## :warning: Usage policies {.scrollable}

- Most LLMs come with usage policies that warrant careful consideration, for example:

    - [OpenAI's usage policy](https://openai.com/policies/usage-policies)
    - [OpenAI's sharing and publication policy](https://openai.com/policies/sharing-publication-policy)

- Provided that the usage of LLMs is in line with the suitable policy, it is prudent to: 

    - __document__ the usage, by e.g. keeping a log of prompts used
    - __communicate__ the usage to the relevant stakeholders by accompanying the generated content with a disclaimer, such as:

    > This text was generated in part with __INSERT MODEL NAME AND VERSION__. Upon generating draft language, the author reviewed, edited, and revised the language to their own liking and takes ultimate responsibility for the content of this publication

## :rocket: Tips and tricks -- prompt engineering

To get better at prompt engineering, practice is key!
...
## :rocket: GenAI tools that I find useful
